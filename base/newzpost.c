...
typedef struct {
	newzpost_data *data;
	newzpost_threadinfo *threadinfo;
	queue *fifo;
}
poster_thread_arg;

static int post_text_file(newzpost_data *data, SList *file_list);

static int encode_and_post(newzpost_data *data, SList *file_list,
			    SList *parfiles);

static void post_file(newzpost_data *data, queue *fifo, file_entry *file_data,
		      int filenumber, int number_of_files, const char *filestring);

static void *poster_thread(void *arg);

static Buff *make_subject(Buff *subject, newzpost_data *data,
	int filenumber, int number_of_files, const char *filename,
	int partnumber, int number_of_parts, const char *filestring);

static SList *preprocess(newzpost_data *data, SList *file_list);
...
int newzpost(newzpost_data *data, SList *file_list) {
	int retval;
	SList *parfiles = NULL;
...
static int encode_and_post(newzpost_data *data, SList *file_list,
	...
	newzpost_threadinfo *threadinfo_array =
		(newzpost_threadinfo *) malloc(data->threads * sizeof(newzpost_threadinfo));
...
static void *poster_thread(void *arg)
{
	...
	newzpost_data *data = arguments->data;
	newzpost_threadinfo *tinfo = arguments->threadinfo;
...
static Buff *make_subject(Buff *subject, newzpost_data *data, int filenumber,
...
static SList *preprocess(newzpost_data *data, SList *file_list) {
	...
	if (data->par != NULL) {
		parfiles = par_newzpost_interface(data, file_list);
		if (data->uuenc == FALSE)
