...
/* Symbolic labels for .newzpostrc keywords */
...
/* The .newzpostrc keywords themselves -- must be in the same order */
...
/* Comment lines for the .newzpostrc -- must be in the same order */
...
static boolean parse_file_parts(newzpost_data *data,
...
void parse_defaults(newzpost_data *data) {
	...
	envopt = getenv("HOME");
	if (envopt != NULL) {
		filename = buff_create(filename, "%s/.newzpostrc", envopt);
		...
		if (file != NULL) {
			...
			/* try to read the old .newzpost */
			filename = buff_create(filename, "%s/.newzpost", envopt);
			file = fopen(filename->data, "r");
			if (file != NULL) {
				linenum = 0;
				while (linenum < 8) {
...
void print_help() {
	version_info();
	printf("\n\nUsage: newzpost [OPTIONS [ARGUMENTS]]"
		" file1 file2 file3...");
	printf("\nOptions:");
	...
	printf("\nPlease see the newzpost manpage for more"
